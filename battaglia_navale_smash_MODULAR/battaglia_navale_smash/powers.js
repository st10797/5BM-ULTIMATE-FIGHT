...
function usePower(pi) {
  ...
  switch (ch.powType) {
    ...
    ────────────────────────────────────────────────────── */
    case 'super_rimbalzo': {
      showPowAnnounce('🎈 SUPER RIMBALZO!', ch.col);
      flashScreen(ch.col, 0.45);
      // Bolly rimbalza in alto con grande velocità
      pp.vy = -pp.ch.jump * 2.2;
      pp.vx = pp.facing * 12;
      pp.invincT = 0.6;
      for (let i = 0; i < MAX_PARTICLES_PER_EXPLOSION; i++) {
        ptcls.push({ x: pp.x + pp.w / 2 + (Math.random() - 0.5) * 20, y: pp.y + pp.h, vx: (Math.random() - 0.5) * 8, vy: -2 - Math.random() * 6, life: 0.7, ml: 0.7, col: ch.col, sz: 4 + Math.random() * 6 });
      }
      addPfx({ type: 'shockwave', x: pp.x + pp.w / 2, y: pp.y + pp.h / 2, r: 0, maxR: 160, life: 0.45, col: ch.col });
      // Applica controlli invertiti all'avversario per 3 secondi
      if (!op.isDead) {
        ...
        setTimeout(() => {
          if (p[1 - pi]) {
            ...
            flt('✓ Controlli normali', p[1 - pi].x, p[1 - pi].y, '#88ff88');
          }
        }, 3000);
      }
      // Danno al contatto durante il rimbalzo
      const bounceIv = setInterval(() => {
        if (!p[pi] || !p[1 - pi]) { clearInterval(bounceIv); return; }
        const dist = Math.hypot((p[pi].x + p[pi].w / 2) - (p[1 - pi].x + p[1 - pi].w / 2), (p[pi].y + p[pi].h / 2) - (p[1 - pi].y + p[1 - pi].h / 2));
        if (dist < 60 && !p[1 - pi].isDead && p[1 - pi].invincT <= 0) {
          p[1 - pi].damage += ch.powDmg;
          p[1 - pi].vx += pp.facing * 15; p[1 - pi].vy = -20; p[1 - pi].hitFlash = 0.35;
          updPct(1 - pi); popDmg(p[1 - pi].x + p[1 - pi].w / 2, p[1 - pi].y, ch.powDmg, '🎈', ch.col);
          spawnHitParticles(p[1 - pi].x + p[1 - pi].w / 2, p[1 - pi].y + p[1 - pi].h / 2, ch.col, MAX_PARTICLES_PER_EXPLOSION);
          shake = 1.2; hitstopFrames = 8; clearInterval(bounceIv);
        }
      }, 30);
      setTimeout(() => clearInterval(bounceIv), 800);
      break;
    }
    ...
    ────────────────────────────────────────────────────── */
    case 'onda_musicale': {
      showPowAnnounce('🎩 ONDA MUSICALE!', ch.col);
      flashScreen(ch.col, 0.4);
      for (let i = 0; i < 3; i++) {
        setTimeout(() => {
          // Onda verso destra
          projs.push({ x: pp.x + pp.w / 2, y: pp.y + pp.h * 0.5, vx: pp.facing * (12 + i * 2), vy: (i - 1) * 2, owner: pi, dmg: Math.round(ch.powDmg / 3), col: ch.col, sz: 14, type: 'onda', life: 2.0, spin: 0 });
          for (let j = 0; j < 5; j++) {
            ptcls.push({ x: pp.x + pp.w / 2, y: pp.y + pp.h * 0.5 + (Math.random() - 0.5) * 20, vx: pp.facing * (8 + Math.random() * 6), vy: (Math.random() - 0.5) * 4, life: 0.3, ml: 0.3, col: ch.col, sz: 3 + Math.random() * 5 });
          }
          addPfx({ type: 'shockwave', x: pp.x + pp.w / 2, y: pp.y + pp.h / 2, r: 0, maxR: 80 + i * 30, life: 0.35, col: ch.col });
        }, i * 150);
      }
      shake = 0.6;
      break;
